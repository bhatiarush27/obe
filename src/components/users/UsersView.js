...
const UsersView = ({ permissions }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (modalTask === "Add") {
        ...
        await axios.put(
          `http://localhost:5001/api/users/${selectedUserId}`,
          userDetails
        );
        alert("User updated successfully");
      }
      ...
      console.error(
        `Error ${modalTask === "Add" ? "adding" : "updating"} user:`,
        error
      );
      alert(`Failed to ${modalTask === "Add" ? "add" : "update"} user`);
    } finally {
  ...
  const handleUserArchive = async (user, archiveStatus) => {
    ...
    if (confirmArchive) {
      try {
        ...
        const response = await axios.put(
          ...
          { ...user, isActive: archiveStatus }
        );
        if (response.status < 400) {
          setUsers(
            users.map((user) =>
              user._id === userId ? { ...user, isActive: archiveStatus } : user
            )
          );
          window.alert(
            `User ${user.isActive ? "archived" : "unarchived"} successfully`
          );
        } else {
          console.error("Failed to archive user:", response.data.error);
          window.alert(
            `Failed to ${user.isActive ? "archive" : "unarchive"} user`
          );
        }
      } catch (error) {
        ...
        <Table>
          <TableHead>
            <TableRow>
              ...
              <TableCell>Level</TableCell>
              <TableCell>Email</TableCell>
              {permissions.includes("userActions") && (
                <TableCell>Actions</TableCell>
              )}
            </TableRow>
          </TableHead>
          <TableBody>
            {users.map((user) => (
              <TableRow key={user._id}>
                ...
                {permissions.includes("userActions") && (
                  <TableCell>
                    <Button onClick={() => handleModalOpen("Edit", user)}>
                      Edit
                    </Button>
                    <Button
                      onClick={() => handleUserArchive(user, !user.isActive)}
                      style={{
                        backgroundColor: "red",
                        color: "white",
                        borderRadius: "10px",
...
const userLevels = [
  {
    ...
    name: "Super Admin",
    includes:
      "Head of the institute, or administrative body at Institute level",
  },
  {
    key: "admin",
    name: "Admin",
    includes:
      "Head of the department, or administrative body at Department level",
  },
  {
    key: "faculty",
    name: "Faculty",
